        ...
        for line in csv_file:
            ...
            for tf, motif in motif_dict.items():
                ...
                for pos, score in pssm.search(sequence, threshold=threshold):
                    start, end = pos, pos + motif_len
                    if 0 <= start <= len(sequence) - motif_len:
                        results.append([gene_name, gene_id, tf, "+", start, end])

                # Reverse strand
                reverse_seq = Seq(sequence).reverse_complement()
                for pos, score in pssm.search(reverse_seq, threshold=threshold):
                    start = len(sequence) - (pos + motif_len)
                    end = len(sequence) - pos
                    if 0 <= start <= len(sequence) - motif_len:
                        results.append([gene_name, gene_id, tf, "-", start, end])
...
def main():
    ...
    parser.add_argument('--tf', nargs='+', required=True, help="List of TFs to scan (e.g. IRF3 IRF7 IRF9)")
    parser.add_argument('--threshold', '-t', type=float, default=7.0, help="PSSM match threshold (default: 7.0)")
